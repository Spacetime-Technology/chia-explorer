...
Backed by the public [coinset.org](https://www.coinset.org) full-node API for chain data and [CoinGecko](https://www.coingecko.com) for XCH price. Supports mainnet (default) and testnet11. Auto-detects network from `xch` or `txch` address prefixes.
...
- Fetch the CLVM puzzle reveal and solution for a spent coin, so you can see what the spend actually did.
...
| `get_puzzle_and_solution` | CLVM puzzle reveal and solution for a coin at its spent height |
